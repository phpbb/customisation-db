...
class ai_validation extends Command
{
    ...
    {
        // Run the command with: php bin/phpbbcli.php custdb:ai_validation 2
        $this
    ...
    private function process_queue(int $queue_id, OutputInterface $output): bool
    {
        ...
        $zip_path = $this->manager->get_settings()['file.upload.path'] . $revision['revision_attachment'];
    ...
    // Different prompt files depending on the type
    private function get_ai_prompt_file_from_type(string $prompt_type, int $contribution_type)
    {
        switch ($contribution_type)
        {
            case $this->manager::TYPE_EXTENSIONS:
                return ($prompt_type === "system") ? 'extension.system.prompt.txt' : 'extension.user.prompt.txt';
                break;
            case $this->manager::TYPE_STYLES:
                return ($prompt_type === "system") ? 'style.system.prompt.txt' : 'style.user.prompt.txt';
                break;
            case $this->manager::TYPE_TRANSLATIONS:
                return ($prompt_type === "system") ? 'translation.system.prompt.txt' : 'translation.user.prompt.txt';
                break;
        }
    }
    ...
    {
        $system_prompt = $this->load_prompt_file($this->get_ai_prompt_file_from_type('system', (int) $contribution['contribution_type']));
        $user_prompt = $this->load_prompt_file($this->get_ai_prompt_file_from_type('user', (int) $contribution['contribution_type']));
    ...
    {
        $path = __DIR__ . '/prompts/' . $file_name;
        if (!file_exists($path))
        {
            return '';
        }
