...
class titania_faq extends titania_database_object
{
	 ...
	 * @param int $faq_id
	 */
	public function __construct($faq_id = false, $contrib_type = 0)
	{
	...
	public function faq_details()
	{
		...
		$template->assign_vars(array(
			...
			'U_OTHERS_FAQ'		=> append_sid(TITANIA_ROOT . $this->contrib_type . '/index.' . PHP_EXT, 'mode=view&amp;contrib_id=' . $this->contrib_id),
		));
	}
	...
	public function similar_faq()
	{
		...
		$sql_ary = array(
			...
			'FROM'		=> array(
				CUSTOMISATION_CONTRIBS_FAQ_TABLE => 'f',
			),
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$template->assign_block_vars('similarfaq', array(
				'U_FAQ'		=> append_sid(TITANIA_ROOT . $this->contrib_type . '/index.' . PHP_EXT, 'mode=view&amp;faq_id=' . $row['faq_id']),
	...
	public function faq_list($contrib_id)
	{
		...
		// Build a pagination
		$pagination->build_pagination($titania->page);
