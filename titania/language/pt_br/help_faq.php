...
$help = array(
	...
	array(
		...
		1 => 'Você pode ir diretamente ao tipo de modificação desejada baseado em qual categoria a contribuição é baseada (ferramentas, comunicação, segurança, entretenimento, etc) ou usando o recurso de busca no topo da página. Se estiver usando o recurso de busca, você pode usar coringas e buscar pelo nome da contribuição (ou parte do nome), bem como o autor da contribuição. Após encontrar a customização em que está interessado você será levado para a página “Detalhes da customização”, onde pode encontrar o download da versão atual da customização bem como as versões anteriores dela dentro da seção “Revisões”.'
	),
	array(
		0 => 'Encontrando um estilo',
		1 => 'Semelhante a procura de modificações, a Titania também permite que você localize estilos, pacotes de smilies, imagens de ranks e outros itens. O recurso de busca também permite que você use coringas e busque apenas por nomes de autores. Após encontrar o item que desejado, você será levado a página “Detalhes da contribuição” onde você pode encontrar o download da versão atual do item, bem como as versões anteriores do item na seção “Revisões”.'
	),
	array(
		0 => '--',
	...
	array(
		...
		1 => 'Uma vez que uma customização é aceita na base de dados, ela é encaminhada a equipe relevante que validará sua contribuição. Você pode receber uma mensagem informando que a sua customização foi rejeitada. Por favor, não se preocupe. Sabemos que algumas coisas passam despercebidas. Não há nada com o que se preocupar. A mensagem que você recebe conterá itens que foram encontrados. Estes itens podem sugerir alterações no código ou em imagens e podem até mesmo sugerir alterações na “interatividade com o usuário”. De modo geral, sugestões de “interatividade com o usuário” são apenas... sugestões. A parte mais importante de qualquer customização é a segurança, e não como ela exibida ao usuário final.<br /><br />Se nenhum item foi encontrado durante a validação de sua contribuição, você receberá uma MP dizendo que sua contribuição foi aceita na base de dados. Agora é hora de relaxar um pouco e deleitar-se com o conhecimento de que você fez uma contribuição para a comunidade open source.<br /><br />Não importa o resultado da validação, agradecemos o tempo e o esforço que você exerceu em compartilhar sua contribuição.'
	),
	array(
		0 => 'Quem irá validar a minha contribuição?',
		1 => 'Se for uma modificação será validada pela equipe de MODs e os validadores junior de MODs ou, ocasionalmente, um membro da equipe de desenvolvimento. Se for um estilo, será validado pela equipe de estilos e pelos validadores junior de estilos. Se for um conversor, será validado por um membro da equipe de suporte ou da equipe de desenvolvimento. Se for uma bridge, será validado por um membro da equipe de MODs ou da equipe de desenvolvimento. Traduções são verificadas pelo gerenciador de traduções e do IST, ameeck. Ferramentas oficiais são testadas e criadas pelas equipes do phpBB.com.'
	),
);
